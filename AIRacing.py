...
class Car:
    ...
    def move(self, f, l, r):
        ...
        self.driftSpeed = math.sqrt((self.x_speed ** 2) + (self.y_speed ** 2))
        self.driftAngle = ((math.degrees(math.atan2(-1*self.y_speed,self.x_speed)) - 90 + self.angle)%360)/18
        if self.driftAngle > 10:
            self.driftAngle = -1 * (10 - (self.driftAngle%10))
        if f:
            self.Speed += self.Acc
        else:
            self.Speed -= self.Acc
...
def main(genomes, config):
    ...
    while run:
        ...
        for x, car in enumerate(cars):
            ...
            output = nets[x].activate((car.echo(0), car.echo(1), car.echo(2), car.echo(3), car.echo(4),  car.Speed, car.driftSpeed, car.driftAngle))
